# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Overview

This is a personal recipe collection - markdown files containing food recipes. No build system, tests, or dependencies.

## Recipe Format

Each recipe is a standalone `.markdown` file with:

- H1 title (recipe name)
- Original recipe/source link at the top (if adapted from elsewhere)
- `## Ingredients` section (bulleted list)
- `## Directions` section (numbered steps)
- Personal modifications, notes, or variations at the bottom

## Naming Convention

Files use lowercase with hyphens: `recipe-name.markdown`

Prefix patterns indicate cooking method:

- `pressure-cooker-*` - Instant Pot/pressure cooker recipes
- `slow-cooker-*` - Crock-Pot recipes

## Workflow Tips

When fetching recipe details from external URLs using Playwright MCP, use a subagent (Task tool) to reduce token usage in the main conversation.
